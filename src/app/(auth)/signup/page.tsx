...
import {
    ...
    ShieldCheck, Lock, Star
} from 'lucide-react';
import { motion } from 'framer-motion';

// 👉 Backend Action & NextAuth Imports
...
export default function SignupPage() {
    ...
    return (
        <div className="min-h-screen flex w-full font-sans text-[#304250] bg-[#f8fafc] items-center justify-center p-4 relative overflow-hidden">
            {/* Background decorative glow - Matches Login */}
            <div className="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] rounded-full bg-[#20A46B]/10 blur-[100px] pointer-events-none"></div>

            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: 'easeOut' }} className="w-full max-w-[480px] bg-white p-8 sm:p-10 rounded-[32px] shadow-[0_20px_60px_-15px_rgba(48,66,80,0.08)] border border-[#304250]/10 relative z-10">

                {/* Centered Header with Exact Logo Path */}
                ...
                {successMessage && <div className="mb-6 p-3 bg-[#20A46B]/10 border border-[#20A46B]/30 text-[#20A46B] rounded-xl text-sm font-medium text-center">{successMessage}</div>}
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" placeholder="Ali Raza" {...register('fullName')} className="w-full h-11 px-4 rounded-xl border border-[#304250]/20 text-sm bg-gray-50/50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#20A46B] focus:border-transparent transition-all placeholder:text-[#304250]/40 text-[#304250] font-medium" />
                        ...
                        <div>
                            ...
                            <input type="text" placeholder="Ali Gadgets" {...register('storeName')} className="w-full h-11 px-4 rounded-xl border border-[#304250]/20 text-sm bg-gray-50/50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#20A46B] focus:border-transparent transition-all placeholder:text-[#304250]/40 text-[#304250] font-medium" />
                    ...
                    <div>
                        ...
                        <input type="email" placeholder="ali@example.com" {...register('email')} className="w-full h-11 px-4 rounded-xl border border-[#304250]/20 text-sm bg-gray-50/50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#20A46B] focus:border-transparent transition-all placeholder:text-[#304250]/40 text-[#304250] font-medium" />
                    ...
                    <div>
                        ...
                        <div className="flex relative rounded-xl shadow-sm">
                            <select {...register('countryCode')} className="h-11 py-0 pl-3 pr-7 border border-[#304250]/20 bg-gray-50/50 text-[#304250] text-sm rounded-l-xl focus:ring-[#20A46B] focus:border-[#20A46B] font-medium">
                                <option value="+92">PK (+92)</option>
                                <option value="+1">US (+1)</option>
                            </select>
                            <input type="text" placeholder="3001234567" {...register('whatsapp')} className="flex-1 w-full h-11 px-4 rounded-r-xl border-y border-r border-l-0 border-[#304250]/20 text-sm bg-gray-50/50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#20A46B] focus:border-transparent transition-all placeholder:text-[#304250]/40 text-[#304250] font-medium" />
                        </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <input type={showPassword ? "text" : "password"} placeholder="••••••••" {...register('password')} className="w-full h-11 pl-4 pr-10 rounded-xl border border-[#304250]/20 text-sm bg-gray-50/50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#20A46B] focus:border-transparent transition-all placeholder:text-[#304250]/40 text-[#304250] font-medium" />
                            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3 text-[#304250]/40 hover:text-[#20A46B] transition-colors">
                    ...
                    <div className="flex items-center justify-center gap-4 text-[11px] text-[#304250]/60 font-medium pt-1">
                        <span className="flex items-center gap-1"><Lock size={12} className="text-[#304250]/40" /> No credit card required</span>
                        <span className="w-1 h-1 bg-[#304250]/20 rounded-full"></span>
                        <span className="flex items-center gap-1"><ShieldCheck size={12} className="text-[#20A46B]" /> Secure & encrypted</span>
                    </div>
                </form>
                ...
                <p className="mt-8 text-center text-sm font-medium text-[#304250]/60">
                    Already have an account? <Link href="/login" className="text-[#20A46B] hover:opacity-80 font-bold transition-opacity">Log in</Link>
                </p>
            </motion.div>
        </div>
    );
